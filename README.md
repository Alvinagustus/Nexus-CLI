...
## --> Contribute via CLI
**If you use Ubuntu 22 please read the Troubleshoot section...** You might need a free +8GB RAM, you'd better to test it yourself since the testnet is under cunstructions
...
5- Build (This one gonna be a long install so be patient)
```
...
6- `nexus-network` command doesnt read the new GLIBC, so run `/opt/glibc-2.39/lib/ld-linux-x86-64.so.2 --library-path /opt/glibc-2.39/lib:/lib/x86_64-linux-gnu:/usr/lib/x86_64-linux-gnu /usr/local/bin/nexus-network`. For example: 
```
...
7. ***If you already have account, address, and node id run this command :***
```
